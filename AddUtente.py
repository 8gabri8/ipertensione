...
class AddUtente(tk.Frame):
    def __init__(self, parent, controller, utente):
        ...
        if (utente != None):
            ...
            Label(self, text='Dati Utente', foreground='black', font=('Helvetica', 30)).pack(pady=5)                
            ...
            Label(self, text=f'Codice:\t{id}', foreground='black', font=controller.font).pack(pady=5)

            psw = id.lower() #pasw è id in minuscolo self, text="Inserisci ID:" ,foreground="black", font=controller.font
            Label(self, text=f'psw:\t{psw}', foreground='black', font=controller.font).pack(pady=5)

            Label(self, text="Nome:" ,foreground="black", font=controller.font).pack(pady=5)
            nome = StringVar()
            ttk.Entry(self, textvariable=nome, width=30, font=controller.font).pack(pady=5)

            Label(self, text='Cognome:', foreground='black', font=controller.font).pack(pady=5)
            cognome = StringVar()
            ttk.Entry(self, textvariable=cognome, width=30, font=controller.font).pack(pady=5)

            Label(self, text='mail:', foreground='black', font=controller.font).pack(pady=5)
            mail = StringVar()
            ttk.Entry(self, textvariable=mail, width=30, font=controller.font).pack(pady=5)

            Label(self, text="Data nascita:", foreground='black', font=controller.font).pack(pady=5)
            #date_format = "%Y-%m-%d"
            ...
            if (utente.get_tipo_utente_aggiunto() == 'P'): # se aggiungo paziente
                ...
                Label(self, text='Codice Medico di riferimento:', foreground='black', font=controller.font).pack(pady=5)
            ...
            ttk.Button(self, text='Aggiungi', style='Custom.TButton', command=lambda: controller.crea_utente(utente, id, nome.get(), cognome.get(), mail.get(),
                                                                                cal_dataN.selection_get().strftime('%Y-%m-%d'), id_med_ref.get(), psw, parent)).pack(side="right", padx=40, pady=10)

        ####### INDIETRO ################
            def indietro():
                controller.show_frame("HomeResp", parent, utente)  

            ttk.Button(self, text="Indietro", style='Custom.TButton', command=indietro).pack(side="left", padx=40, pady=10)