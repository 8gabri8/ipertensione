...
class ModPaz(tk.Frame):
    ...
    def __init__(self, parent, controller, utente): #utente è medico
        ...
        if(utente != None):
            ...
            frame0 = LabelFrame(self, text="Paziente")
            frame0.pack(fill="x")
            Label(frame0, text=f"Stai osservando il paziente:\n{dati[0][1]} {dati[0][2]}, ID: {dati[0][0]}").pack()

#################### TABELLA PAT PREGRESSE ###################

            frame2 = LabelFrame(self, text="Patologie pregresse")
            frame2.pack(fill="x")
            ...
            tabella_preg = ttk.Treeview(frame2, columns=colonneP, show="headings") #la prima riga della tabell ha nome dellecoloenn

            tabella_preg.column('Nome', anchor=CENTER)
            tabella_preg.heading('Nome', text='Nome')
            tabella_preg.column('Inizio', anchor=CENTER)
            tabella_preg.heading('Inizio', text='Inizio')
            tabella_preg.column('Fine', anchor=CENTER)
...
##################### TABELLA PAT CONCOMITANTI ######################################
            ...
            frame3 = LabelFrame(self, text="Patologie concomitanti")
            frame3.pack(fill="x")
            ...
            tabella_conc = ttk.Treeview(frame3, columns=colonneC, show="headings") #la prima riga della tabell ha nome dellecoloenn

            tabella_conc.column('Nome', anchor=CENTER)
            tabella_conc.heading('Nome', text='Nome')
            tabella_conc.column('Inizio', anchor=CENTER)
            tabella_conc.heading('Inizio', text='Inizio')
            tabella_conc.column('Tipo', anchor=CENTER)
            tabella_conc.heading('Tipo', text='Tipo')

            tabella_conc.grid(row=0, column=0, pady=10)
            ...
            def OnDoubleClick_mod_pat_conc(event):
                ...
                # Create a LabelFrame for the patient information
                frame = LabelFrame(top, text="Patologie Concomitante")
                frame.pack(fill="both", expand=True, padx=10, pady=10)
                ...
                if(values[2] == "Segnalata"):
                    ...
                    Button(frame, text="Accetta", command=lambda: controller.accetta_pat_conc(utente, values[0], values[1], top, parent)).grid(row=1, column=0, pady=10)
                    
                    Button(frame, text="Rifiuta", command=lambda: controller.cancella_pat_conc(utente, values[0], values[1], top, parent)).grid(row=1, column=1, pady=10)
                    ...
                    Label(frame, text="Data Fine:").grid(row=2, column=0)
                    cal_fine = Calendar(frame, date_pattern='yyyy-mm-dd') # Change the date pattern here
                    cal_fine.grid(row=2, column=1)
                    
                    # Create a button to close the Toplevel widget
                    Button(frame, text="Modifica", command=lambda: controller.modifica_pat_conc(utente, datetime.strptime(cal_inizio.selection_get().strftime('%Y-%m-%d'), '%Y-%m-%d'),
                                                                                                cb_pat.get(), values[1], values[0], top, parent)).grid(row=4, column=0, pady=10)

                    Button(frame, text="Rendi Pregressa", command=lambda: controller.conc2preg(utente, datetime.strptime(values[1], '%Y-%m-%d'),
                                                                                                datetime.strptime(cal_fine.selection_get().strftime('%Y-%m-%d'), '%Y-%m-%d'),
                                                                                                values[0], top, parent)).grid(row=4, column=1, pady=10)
...
##############INDIETRO #########################à
            ...
            Button(self, text="Indietro", command=indietro).pack(side="left", padx=10)
